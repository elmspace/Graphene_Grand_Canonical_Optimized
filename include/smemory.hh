...
void create_array(double*****& array, int n1, int n2, int n3, int n4, int n5, const char *name){

        double *data        = (double *) smalloc(n1*n2*n3*n4*n5*sizeof(double),name);
        double **plane      = (double **) smalloc(n1*n2*n3*n4*sizeof(double *),name);
        double ***cube      = (double ***) smalloc(n1*n2*n3*sizeof(double **),name);
        double ****teseract = (double ****) smalloc(n1*n2*sizeof(double ***),name);
        array               = (double *****) smalloc(n1*sizeof(double ****),name);
        
        for (int i=0; i<n1; i++){
                array[i] = &teseract[i*n2];
                for (int j = 0; j<n2; j++){
                        teseract[i*n2+j] = &cube[(i*n2+j)*n3];
                        for (int k = 0; k<n3; k++){
                                cube[(i*n2+j)*n3+k] = &plane[((i*n2+j)*n3+k)*n4];
                                for (int l = 0; l<n4; l++){
                                        plane[((i*n2+j)*n3+k)*n4+l] = &data[(((i*n2+j)*n3+k)*n4+l)*n5];
                                }
                        }
                }
        }
